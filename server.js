...
var http, httpProxy, proxyServer, _;
...
proxyServer = function(req, res, proxy) {
  ...
  if(!_.isEmpty(process.env.ALLOWED_ORIGINS)){
    if(!_.contains((process.env.ALLOWED_ORIGINS || '').split(','),req.headers.origin)){
      console.log(req.headers.origin + ' not authorized');
      res.write('hello https\n');
      res.end();
      return;
    }
...
httpProxy.createServer(proxyServer).listen(process.env.PORT || 5001);
